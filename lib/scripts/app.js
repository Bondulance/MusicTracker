...
document.getElementById("artist_form").addEventListener("submit", async (e) => {
    ...
    const album = await APIController.getArtistAlbums(token, artistName);
...
let artistRegion = document.getElementById("artist_region")
...
const resultGenres = document.createElement("p");
resultGenres.className = "genre_result";
resultGenres.appendChild(document.createTextNode(`${artist.genres.slice(0,3)}` || ''));

const addButton = document.createElement("button");
addButton.className = "add_button";
addButton.type = "submit";
addButton.onclick = "addArtist_to_DB;";
addButton.textContent = "Add to Month";

const vertDiv = document.createElement('div');
vertDiv.className = "vert";
vertDiv.appendChild(resultName);
vertDiv.appendChild(resultGenres);
vertDiv.appendChild(addButton);

resultDiv.appendChild(resultIMG);
resultDiv.appendChild(vertDiv);


artistRegion.appendChild(resultDiv);
